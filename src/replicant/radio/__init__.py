...
- ``replicant.radio.sky`` — astrophysical: 21 cm global signal, foregrounds,
  point sources (+ the simplest uniform ``SkyOperator``).
- ``replicant.radio.environment`` — ionosphere, atmospheric emission, ground
  pickup, RFI.
- ``replicant.radio.instrument`` — beam, noise-wave / reflection terms, bandpass,
  gain, CW calibration tone, thermal noise, self-generated EMI, digitisation.
- ``replicant.radio.backend`` — flagging, averaging.

A forward model composes them with the two core combinators, following the
canonical signal-path graph (``replicant.radio.graph``; RFI enters as a
...
from replicant.radio.backend import BackendOperator, FlaggingOperator, MomentRFIFlaggingOperator
from replicant.radio.environment import (
    AtmosphericEmissionOperator,
    GroundPickupOperator,
    IonosphereOperator,
    RFIOperator,
)
from replicant.radio.filters import (
    AbstractLinearFilter,
    FourierBandFilter,
    SiderealFilter,
    SkySpaceFilter,
)
from replicant.radio.instrument import (
    ADCOperator,
...
from replicant.radio.surrogate import NeuralOperator
...
from replicant.radio.graph import RADIO_GRAPH, assemble  # noqa: E402  (needs operators above)

__all__ += ["RADIO_GRAPH", "assemble"]

from replicant.radio.graph import _validate_registrations as _v  # noqa: E402
