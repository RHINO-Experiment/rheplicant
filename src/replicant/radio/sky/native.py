...
Completes the maturity ladder of :mod:`replicant.radio.sky.projection`
...
the exact transpose that :class:`~replicant.radio.filters.SkySpaceFilter`
map-making requires.
...
the :class:`~replicant.radio.sky.projection.AbstractSkyProjector` seam. It is
imported lazily so replicant-telescope's dependencies are unchanged.
...
from replicant.radio.sky.projection import AbstractSkyProjector
...
class NativeLimTODProjector(AbstractSkyProjector):
    ...
    :class:`~replicant.radio.sky.projection.LimTODProjector`):
