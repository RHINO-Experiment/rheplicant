...
:mod:`~replicant.radio.sky.model`). A projector maps sky maps to the
...
``adjoint``, which :class:`~replicant.radio.filters.SkySpaceFilter` reuses for
...
- :class:`LimTODProjector` — oracle bridge to numpy limTOD via
  ...
  (:class:`~replicant.radio.sky.native.NativeLimTODProjector`).
...
class LimTODProjector(AbstractSkyProjector):
    ...
    (:class:`~replicant.radio.sky.native.NativeLimTODProjector`) is tested
