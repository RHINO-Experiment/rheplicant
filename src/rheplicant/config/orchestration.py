...
def canonical_layers(
    ...
    *,
    ...
    layer_deletions: Mapping[LayerIdentity, Sequence[DeletionRecord]] | None = None,
) -> _Canonical:
    ...
    if supplied is None:
        ...
        return _Canonical(
            layers=tuple(enumeration.layers),
    ...
    if not layers or layers[0].kind != "base" or len(set(identities)) != len(identities):
        raise ConfigError(
            "supplied canonical layers begin with exactly one base layer "
            "and carry unique identities."
        )
    if set(layer_origins) != set(identities) or set(layer_deletions) != set(identities):
        raise ConfigError(
...
def validate_base_variant_targets(canonical: _Canonical) -> None:
    ...
    declared = (
        f"this document declares {sorted(names)}" if names else "this document declares no variants"
    )
...
def run_text_preflight_all_layers(
    ...
    *,
    ...
    with using_dimension_registry_snapshot():
        return _run_text_preflight_all_layers(canonical, trace=trace, environments=environments)
    ...
    for layer in canonical.layers:
        ...
        if not failed_structure:
            ...
            with using_dimension_environment(environment):
                found = sweep(CHECKS, layer_document, label=_LABEL, sections=_SECTIONS).findings
...
def select_build_layers(
    ...
    *,
    ...
    if not names:
        raise ConfigError(
            f"variant {variant!r} was requested but this document declares no variants."
        )
    raise ConfigError(
        f"variant {variant!r} is not declared; this document declares {sorted(names)}."
    )
...
def prepare_layer_through_built(
    ...
    *,
    ...
    origin_lookup = (
        None if origins is None else _origin_lookup_for(layer.mutable_document(), origins)
    )
    with (
        using_dimension_environment(environment),
        using_resolution_audit(layer.identity, trace, origin_lookup, capture),
    ):
        configured = _build_with_axes(
            layer.mutable_document(),
            base_dir=base_dir,
            previous=previous,
    ...
    return PreparedLayer(layer=layer, configured=configured, declared_runs=())
...
def _empty_views():
    return ParsedOptions(execution=MappingProxyType({}), resolved=MappingProxyType({}))


def _record_parsed_projection(trace, seen, layer, index, spec, resolved, checks):
    key = (layer, index, spec.name, spec.kind, spec.variant)
    if trace is None or key in seen:
        return
...
def _configured_for_declaration(configured, layer):
    """Keep target-layer facts while binding run-default evidence to its declaration."""
    audit = configured.context.audit
    if audit is None or audit.layer == layer:
        return configured
    context = dataclasses.replace(
        configured.context,
        audit=ResolutionAudit(layer, audit.trace),
    )
    return configured._replace(context=context)
...
def parse_declared_schedules_once(
    ...
    *,
    ...
    cumulative = Report(
        findings=tuple(
            finding for built in built_layers for finding in built.configured.report.findings
        )
    )
    parsed_layers = []
    recorded_projections = set()
    for built in built_layers:
        layer = built.layer
        specs = parse_runs(
            layer.mutable_document().get("runs"),
            audit=built.configured.context.audit,
        )
        parsed = []
            ...
            if target is None:
                parsed.append(
                    ParsedRun(
                        ...
                        spec=spec,
                        parsed=_empty_views(),
                        declaration_layer=layer.identity,
                    )
                )
                _record_parsed_projection(
                    trace,
                ...
                continue
            parse_configured = _configured_for_declaration(
                target.configured,
                layer.identity,
            )
            if spec.expect == "refuse":
                try:
                    parsed_run = parse_run(
                        spec,
                        parse_configured,
                        index=index,
                        layer=target.layer,
                    ...
                    parsed.append(
                        ParsedRun(
                            ...
                            parsed=ParsedOptions(
                                execution=_CapturedParse(error), resolved=MappingProxyType({})
                            ),
                            declaration_layer=layer.identity,
                        )
                    )
                    ...
                    if target.layer.identity != layer.identity:
                        _record_parsed_projection(
                            trace,
                            recorded_projections,
                    ...
                    checks = handler_for(spec.kind).deferred_checks
                    _record_parsed_projection(
                        trace,
                        recorded_projections,
                        layer.identity,
        ...
        parsed_layers.append(
            PreparedLayer(layer=layer, configured=built.configured, declared_runs=tuple(parsed))
        )
...
def complete_all_postflight(
    ...
    *,
    ...
    for built in parsed_layers:
        ...
        configured = _through_priced(built.configured, layer=identity, trace=trace)
        if trace is not None:
            if not all(
                callable(getattr(trace, method, None))
                for method in (
                    "record_deletion",
                    "record_resource",
                    "record_seed",
                ...
                trace.freeze_layer(
                    identity,
                    {
                        ...
                        "execution_runs": tuple(_descriptor(parsed) for parsed in execution_runs),
                    },
                )
                completed.append(
                    PreparedLayer(
                ...
                continue
            audit_trace = cast(ResolutionAuditSink, trace)
            for deletion in deletions_for(built.layer, deletions):
                audit_trace.record_deletion(
                    identity,
                    {
                        "path": deletion.path,
                        "origin": {
                            "kind": deletion.origin.kind,
                            "name": deletion.origin.name,
                        },
                    },
                )
            effective_document = to_json_value(configured.document)
            assert isinstance(effective_document, Mapping)
            trace.freeze_layer(
                identity,
                {
                    "effective_document": effective_document,
                    "origins": parallel_origins(
                        effective_document, origins_for(built.layer, origins)
                    ),
                    "declared_runs": tuple(_descriptor(parsed) for parsed in built.declared_runs),
                    "execution_runs": tuple(_descriptor(parsed) for parsed in execution_runs),
                    "audit": layer_audit_row(
                        audit_trace,
                        identity,
                        configured=configured,
                    ),
        ...
        completed.append(
            PreparedLayer(
                layer=built.layer, configured=configured, declared_runs=built.declared_runs
            )
        )
...
def _prepare_document_with_capture(
    ...
    *,
    ...
    if scope not in ("selected", "all_layers"):
        raise ValueError(f"scope is 'selected' or 'all_layers'; got {scope!r}.")
    if scope == "selected" and layers is None and variant is not None:
        ...
        variant = None
    canonical = canonical_layers(
        document, supplied=layers, layer_origins=layer_origins, layer_deletions=layer_deletions
    )
    ...
    parsed_layers = parse_declared_schedules_once(built_layers, scope=scope, trace=trace)
    execution_runs = base_parsed_schedule(parsed_layers) if scope == "all_layers" else ()
    completed_layers = complete_all_postflight(
        parsed_layers,
        execution_runs=execution_runs,
        trace=trace,
        origins=canonical.origins,
        deletions=canonical.deletions,
    )
...
def execute_one_parsed(
    ...
    *,
    ...
    if len(matches) > 1:
        raise ConfigError(
            f"runs[{parsed.name!r}]: the {label} layer appears twice in this prepared document."
        )
...
def execute_prepared(
    ...
    *,
    ...
    return ExecutionRecord(
        ...
        completed_boundaries=(() if trace is None else tuple(trace.completed_boundaries())),
    )
