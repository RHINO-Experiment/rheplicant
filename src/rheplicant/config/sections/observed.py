...
def _predicted_shape(fit_twin: Any, space: Any, state: Any) -> tuple[int, ...]:
    ...
    **Dropping the bind is an EQUIVALENT MUTANT, and deliberately left as
    ...
    def predict(twin_: Any, state_: Any):
        base = space.bind(twin_, dict(space.initial_values())) if space is not None else twin_
...
def _realise(
    ...
    *,
    ...
    if kind not in _REALISE_KINDS:
        raise ConfigError(f"{where}.kind: {kind!r} is not one of {list(_REALISE_KINDS)}.")
    if kind == "none":
        check_unknown_keys(where, dict(spec), frozenset({"kind"}), label="kind: none")
    ...
    if kind == "homoscedastic":
        ...
        check_unknown_keys(
            where, dict(spec), frozenset({"kind", "sigma", "seed"}), label="kind: homoscedastic"
        )
        if "sigma" not in spec:
            raise ConfigError(
                f"{where}: kind: homoscedastic requires "
                "sigma: -- the scatter that goes INTO the data."
            )
        destination = DestinationDescriptor(f"{where}.sigma", "config_path", f"{selector}.sigma")
        ...
        return (HomoscedasticNoise(sigma).realise(prediction, key=key), seed, kind)
    if kind == "radiometer":
        from rheplicant.inference import RadiometerNoise

        check_unknown_keys(where, dict(spec), frozenset({"kind", "seed"}), label="kind: radiometer")
        ...
        return (RadiometerNoise(float(width), float(tau)).realise(prediction, key=key), seed, kind)
    # from_model
    check_unknown_keys(where, dict(spec), frozenset({"kind", "seed"}), label="kind: from_model")
...
def _one(
    ...
    *,
    ...
    if "file" in spec:
        check_unknown_keys(where, dict(spec), frozenset({"file"}), label="the file form")
        destination = DestinationDescriptor(
            f"{document_path}.file", "config_path", f"{selector}.file"
        )
        resolved = resolve_value({"file": dict(spec["file"])}, context, destination=destination)
        ...
        return data, {}, {"from": "file", "twin": None, "realise": None, "seed": None}
    if spec.get("from") != "simulation":
        raise ConfigError(
            f"{where}: is {{from: simulation, ...}} or {{file: {{...}}}}; got {dict(spec)!r}."
        )
    check_unknown_keys(where, dict(spec), _SIM_KEYS, label="a simulation")
    choice = (
        spec["twin"] if "twin" in spec else context.use_default("inference.observed[].twin", "full")
    )
    if choice not in ("full", "fit"):
        raise ConfigError(f"{where}.twin: is full or fit (default full); got {choice!r}.")
    base = twin if choice == "full" else fit_twin
    at_spec = spec.get("at") or {}
    if not isinstance(at_spec, Mapping):
        raise ConfigError(f"{where}.at: is a mapping of latent -> truth value; got {at_spec!r}.")
    if at_spec and space is None:
        raise ConfigError(
            f"{where}.at: names latents, and this document declares no inference.parameters."
        )
    ...
    return data, at_values, {"from": "simulation", "twin": choice, "realise": kind, "seed": seed}
...
def build_observed(
    ...
    *,
    ...
    if not isinstance(section, Mapping):
        raise ConfigError(f"inference.observed: is a mapping; got {section!r}.")
    if "from" in section or "file" in section:
        ...
        for name, spec in section.items():
            ...
            if not isinstance(spec, Mapping) or not ("from" in spec or "file" in spec):
                raise ConfigError(
                    f"inference.observed.{name}: is {{from: simulation, ...}} "
                    f"or {{file: {{...}}}}; got {spec!r}."
                )
            named[name] = spec
            destinations[name] = (f"inference.observed.{name}", "inference.observed.*")
        if not named:
    ...
    if "primary" in entries:
        ...
        primary = None
    return ObservedBuild(entries=entries, primary=primary, at=at, records=records)
