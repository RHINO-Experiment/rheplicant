...
class TestSigmaFamiliesWidensPerRecord:
    """A1.2's fix for BLOCKER 2 (Plan 3C fix round), read in
    ``_t2c_generated``'s own docstring.  Two or more NAMED records with none
    of them literally called ``primary`` used to resolve no primary at all
    and stand C18.kind down exactly as an absent ``observed:`` does --
    silently, on a document that can carry the same family mismatch
    :data:`FAMILIES`' own refusing rows REFUSE.  ``_sigma_families`` now
    calls :func:`_t2c_generating_records` once per named record instead of
    ``_t2c_generated`` once for the whole document, so a document like this
    earns a finding, and the finding names the record."""

    def test_a_named_record_with_no_primary_now_earns_a_finding(self):
        """**The bug, made visible.**  Before A1.2 ``mine(document)`` was
        ``frozenset()`` and the document LOADED clean -- the same
        ``NoiseOperator`` x ``radiometer`` mismatch
        :data:`FAMILIES`' own first refusing row REFUSES -- with nobody
        ...
        assert found.severity == REFUSE
        assert found.where == "model.noise"
        assert "'alpha'" in found.message

    def test_the_record_that_did_not_generate_the_data_is_not_named(self):
    ...
    def test_both_named_records_agreeing_earns_no_finding(self):
        """The anti-vacuity partner: the widening is PER RECORD, not a
        blanket refusal of every multi-record document with no ``primary``.
        Both records here are generated by the twin and both AGREE with the
        likelihood -- ``NoiseOperator`` x ``homoscedastic`` is
    ...
    def test_a_record_named_primary_still_checks_only_that_one(self):
        """The constraint A1.2 does NOT relax.  When a record IS literally
        named ``primary``, :func:`_t2c_generating_records` still resolves to
        that one record alone -- unnamed in the message, exactly as before
        ...
        assert "'primary'" not in found.message
        assert "'night'" not in found.message
