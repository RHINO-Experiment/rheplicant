...
class TestCompositionSymbols:
    ...
    def test_svg_dim_state_keeps_full_opacity_and_uses_shape_markers(self, both):
        """Inactive nodes stay readable and remain distinct without fading text."""
        svg = both.to_svg()
        assert (
            "<style>.lit{opacity:1}.wire{opacity:.78}.dim{opacity:1}"
            ".dim rect,.dim line{stroke-dasharray:3 3}"
            ".dim text{font-style:italic}</style>"
        ) in svg
