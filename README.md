...
(2530 tests, 87.7 % coverage, jit+grad+vmap through the full twin; assembly
...
```bash
.venv/bin/python -m pytest          # 2530 tests, ~12 min with coverage
...
`--no-cov` in its own process, so its 361 passing tests contribute nothing to the
