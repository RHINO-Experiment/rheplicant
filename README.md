...
(2230 tests, 94.4 % coverage, jit+grad+vmap through the full twin; assembly
...
```bash
.venv/bin/python -m pytest          # 2230 tests, ~12 min with coverage
...
`--no-cov` in its own process, so its 94 passing tests contribute nothing to the
