...
(2345 tests, 92.9 % coverage, jit+grad+vmap through the full twin; assembly
...
```bash
.venv/bin/python -m pytest          # 2345 tests, ~12 min with coverage
...
`--no-cov` in its own process, so its 209 passing tests contribute nothing to the
