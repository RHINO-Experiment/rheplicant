...
(2223 tests, 95.4 % coverage, jit+grad+vmap through the full twin; assembly
...
```bash
.venv/bin/python -m pytest          # 2223 tests, ~12 min with coverage
...
That split is also why the stated coverage fell from 99.7 % to 95.4 % when the
evidence layer landed, without any code going untested. The second session runs
`--no-cov` in its own process, so its 68 passing tests contribute nothing to the
default report: `sqrtinfo.py`, `factorize.py`, `compressed.py`, `compress.py`,
`memory.py` and `archive.py` show 18–67 % there, and **205 of the default
report's 219 uncovered statements are those six files**. Measured, on the run
