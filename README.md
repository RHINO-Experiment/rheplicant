...
(2735 tests, 82.4 % coverage, jit+grad+vmap through the full twin; assembly
...
```bash
.venv/bin/python -m pytest          # 2735 tests, ~12 min with coverage
...
`--no-cov` in its own process, so its 446 passing tests contribute nothing to the
