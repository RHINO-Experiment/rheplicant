...
(2730 tests, 82.4 % coverage, jit+grad+vmap through the full twin; assembly
...
```bash
.venv/bin/python -m pytest          # 2730 tests, ~12 min with coverage
...
`--no-cov` in its own process, so its 441 passing tests contribute nothing to the
