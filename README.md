...
(2757 tests, 82.8 % coverage, jit+grad+vmap through the full twin; assembly
...
```bash
.venv/bin/python -m pytest          # 2757 tests, ~12 min with coverage
...
`--no-cov` in its own process, so its 460 passing tests contribute nothing to the
